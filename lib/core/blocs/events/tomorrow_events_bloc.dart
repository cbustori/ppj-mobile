...
class TomorrowEventsBloc extends AbstractEventsBloc {
  EventService _eventService = locator<EventService>();
  StreamSubscription eventSubscription;
  final EventsBloc eventsBloc;

  TomorrowEventsBloc({@required this.eventsBloc}) {
    eventSubscription = eventsBloc.listen((eventState) {
      if (eventState == EventsTab.tomorrow && state is EventsUninitialized) {
        add(LoadEvents());
      }
    });
  }
  ...
  Future<List<Event>> fetchEvents(
    ...
    try {
      List<Event> events = await _eventService.getEvents(
          EventDateFilterEnum.TOMORROW, position, filters, currentPage, super.limit);
      return events;
